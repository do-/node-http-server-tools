	...
	* ext2mime () {
	...
	async handle (response) {
		...
		try {
			...
			const contentType = this.getMimeType (filePath); if (contentType) ctx.response.setHeader ('Content-Type', contentType)

			await ctx.write (fs.createReadStream (filePath))
