...
class HttpRequestContext {
	...
	get path () {
		...
		const value = this.url.pathname.split ('/').filter (s => s.length !== 0)
	...
	pipeStream (os) {
...

		{
			...
			if (!response.hasHeader (CONTENT_TYPE)) response.setHeader (CONTENT_TYPE,
				os [HttpRequestContext.CONTENT_TYPE] ??
				this.contentType ??
